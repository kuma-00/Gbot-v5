...
import { Minigame, MinigameConstructor, MinigameData } from "@src/types/minigame";
import { random, speak } from "@src/util";
import { EmbedBuilder, escapeMarkdown, Guild, GuildManager, GuildMember, Message } from "discord.js";

export const minigame: MinigameConstructor = class iddn implements Minigame {
  static gameData = {
    name: "iddn",
    description: "いつどこで誰が何をしたゲーム",
    details: "いつ、どこで、だれが、何をしたを参加者がそれぞれ入力し、そこから文を混ぜて新しい文章を作成します。",
    maxMember: 20,
    minMember: 2,
    joinInMidway: false,
  };
  ...
  async start(): Promise<void> {
    this.progress();
    const result = await Promise.all(this.data.members.map(member => {
...
\`いつ どこで 誰が 何をした\`をスペースで区切って入力してください。
    ...
    const articles = result.map(r => {
      ...
      const users = [when, who, where, what].map(i=>i?.member.displayName || i?.member.user.username).join(" ");
      const text = [when, who, where, what].map(i=>i?.text).join(" ");
      //this.nextPlayer.displayName || this.nextPlayer.username
      return {user:`${escapeMarkdown(users)}`,text:`**${escapeMarkdown(text)}**`}
    });
      const disme = new EmbedBuilder();
    disme.setTitle("IDDN  GAME")
      .setDescription(`完成した文章たち

${articles.map(a=>`${a.user}\n${a.text}`).join("\n\n")}`);
    await this.data.channel.send({ embeds: [disme] });
    speak(this.client,this.data.channel.guild,`完成した文章:${articles.map(a=>a.text).join("\n")}`)
  ...
  async add(member:GuildMember){
    ...
    this.progress();
    return {when:{member,id:member.id, text: when},who:{member,id:member.id, text: who},where:{member,id:member.id, text: where},what:{member,id:member.id, text: what}};
  }
  async progress(){
    if(!this.progressMsg){
      this.progressMsg = await this.data.channel.send({content:`入力状況:${"🟥".repeat(this.data.members.length)}`});
    }else{
      this.progressMsg.edit({content:`入力状況:${"✅".repeat(this.completionMember.length)}${"🟥".repeat(this.data.members.length-this.completionMember.length)}`});
    }
    if(this.data.members.length <= this.completionMember.length && this.progressMsg.deletable){
