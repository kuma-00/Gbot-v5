...
export const command: Command = {
  ...
  autocomplete(client, interaction){
    ...
    interaction.respond([{ name: "choice", value: "choice" }])
  }
};

export const createMessage = (game:MinigameData) =>{
  const gameData = game.gameConstructor.data;
...
`:""}
**・人数**
${gameData.minMember == gameData.maxMember ?gameData.minMember :`${gameData.minMember}~${gameData.maxMember}`}人

**・メンバー**
${game.members.length}人${game.members.length > 0 ? `(${game.members.map(m => m.displayName).join(",")})` : ""}
`)
  ...
  const selects = gameData.ruleData?.map(rule=> {
    return new ActionRowBuilder().addComponents(
      new SelectMenuBuilder()
        ...
        .addOptions(rule.options.map(op => {
  ...
  const start = new ButtonBuilder()
    ...
    .setDisabled(game.isEnd || game.isStart || (game.members.length < gameData.minMember))
    .setLabel("開始");
